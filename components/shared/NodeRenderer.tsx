...
export const NodeRenderer = memo<NodeRendererProps>(({
  ...
  const renderTypeIndicatorContent = () => {
    ...
    return (
      <Badge 
        variant="secondary" 
        className={`text-xs px-2 py-0 h-5 ${getTypeColor(node.type)}`}
      >
        {node.type}
      </Badge>
    );
...
export const ListNodeRenderer = memo<ListNodeRendererProps>(({
  ...
  <NodeRenderer
    {...props}
    node={node}
    renderKey={renderKey || (showPath ? (key, node) => (
  ...
  return (
    ...
    >
      <NodeRenderer
        {...props}
        node={node}
        renderKey={renderKey || (showChildCount && node.childCount > 0 ? (key, node) => (
...
export const useNodeRenderer = (options: {
  ...
  const isNodeHighlighted = useCallback((node: JsonNode) => {
  ...
  const getNodeClassName = useCallback((node: JsonNode) => {
    const baseClasses = [
      'json-node',
      'flex',
      'items-center',
