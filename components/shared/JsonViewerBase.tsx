  ...
  ({
    ...
    return (
      ...
      >
        ...
        {/* Key */}
        <span
          className={`font-medium text-sm text-gray-700 min-w-0 truncate ${compactMode ? 'text-xs' : ''}`}
        >
          {node.key}:
        </span>
        ...
        {/* Value */}
        <span
          className={`text-sm text-gray-600 flex-1 min-w-0 truncate ${compactMode ? 'text-xs' : ''}`}
        >
...
export const JsonViewerBase = memo<JsonViewerBaseProps>(
  ({
      ...
      if (filteredNodes.length > virtualizeThreshold) {
        ...
        return (
          <List
            ...
            itemKey={(index) => filteredNodes[index]?.id ?? index}
          >
            {({ index, style }) => {
              const node = filteredNodes[index];
              return (
    ...
    const renderRawView = () => (
      <Card className="h-full">
        <ScrollArea className="h-full">
          <pre
            className={`p-4 text-sm whitespace-pre-wrap font-mono leading-relaxed ${compactMode ? 'text-xs p-2' : ''}`}
          >
    ...
    return (
      <div
        className={`json-viewer-base flex flex-col h-full ${className}`}
        data-testid="json-viewer-base"
      >
        {/* Header */}
        {showHeader && (
          <div
            className={`flex items-center justify-between gap-2 border-b bg-gray-50/50 ${compactMode ? 'p-2' : 'p-4'}`}
          >
            {/* Left side - Search */}
            {enableSearch && (
              <SearchBar
        ...
        {enableViewModeSwitch && viewerModes.length > 1 && (
          <div className="border-b bg-white">
            <Tabs value={viewMode} onValueChange={handleViewModeChange} className="w-full">
              <TabsList
                className={`grid w-full h-${compactMode ? '10' : '12'} p-1`}
                style={{ gridTemplateColumns: `repeat(${viewerModes.length}, 1fr)` }}
              >
        ...
        >
          <ErrorBoundary fallback={<div>Error loading {viewMode} view</div>}>
            {viewMode === 'tree' || viewMode === 'raw' ? (
              <ScrollArea className="h-full">
                <div className={compactMode ? 'p-2' : 'p-4'}>{renderContent()}</div>
              </ScrollArea>
            ) : (
        ...
        {showFooter && stats && isValid && (
          <div
            className={`border-t bg-gray-50/50 text-xs text-gray-600 ${compactMode ? 'px-2 py-1' : 'px-4 py-2'}`}
          >
            <div className="flex items-center justify-between">
              <span>
                Viewing {stats.type.toLowerCase()} with {stats.keys}{' '}
                {stats.type === 'array' ? 'items' : 'properties'}
              </span>
        ...
        {filteredNodes.length > virtualizeThreshold && (
          <div
            className={`border-t bg-green-50 text-green-700 text-xs ${compactMode ? 'px-2 py-1' : 'px-4 py-2'}`}
          >
            <Eye className="h-3 w-3 inline mr-1" />
            Virtualized rendering active - showing {Math.min(100, filteredNodes.length)} items at
            once
          </div>
        )}
      </div>
