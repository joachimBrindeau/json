...
import { Download, ExternalLink, Image as ImageIcon, Video as VideoIcon, Music } from 'lucide-react';
...
export const MediaRenderer = memo(({ value, detection }: MediaRendererProps) => {
  ...
  if (type === 'image') {
    return <ImageRenderer url={mediaUrl} isBase64={isBase64} onDownload={handleDownload} onOpen={handleOpenInNewTab} />;
  }
...
// Image Renderer
const ImageRenderer = memo(({ url, isBase64, onDownload, onOpen }: {
  url: string;
  isBase64: boolean;
  onDownload: () => void;
  onOpen: () => void;
...
// Video Renderer
const VideoRenderer = memo(({ url, isBase64, onDownload }: {
  url: string;
  isBase64: boolean;
  onDownload: () => void;
}) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent>
          <Button variant="outline" size="sm" onClick={onDownload} className="justify-start w-full">
            <Download className="h-4 w-4 mr-2" />
            Download Video
          </Button>
        </CardContent>
...
// Audio Renderer
const AudioRenderer = memo(({ url, isBase64, onDownload }: {
  url: string;
  isBase64: boolean;
  onDownload: () => void;
}) => {
