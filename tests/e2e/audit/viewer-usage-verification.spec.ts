...
test.describe('Viewer Component Usage Verification', () => {
  test('Homepage (/) - Identify viewer components', async ({ page }) => {
    ...
    const jsonViewer = await page
      .locator(
        '[class*="json-viewer"]:not([class*="ultra"]):not([class*="smart"]):not([class*="simple"]):not([class*="virtual"])'
      )
      .count();
    viewerUsageResults.push({
      route: '/',
  ...
  test('Compare (/compare) - Identify viewer components', async ({ page }) => {
    ...
    const compareComponent = await page
      .locator('[class*="compare"], [data-testid*="compare"]')
      .count();
  ...
  test('Test view mode switching - Homepage', async ({ page }) => {
    ...
    const treeButton = page
      .locator('button:has-text("Tree"), button:has-text("tree"), [data-testid*="tree"]')
      .first();
    const rawButton = page
      .locator('button:has-text("Raw"), button:has-text("raw"), [data-testid*="raw"]')
      .first();
    const flowButton = page
      .locator('button:has-text("Flow"), button:has-text("flow"), [data-testid*="flow"]')
      .first();
