...
test.describe('Smoke Tests @smoke', () => {
  ...
  test('Application loads and basic navigation works', async ({ page }) => {
    ...
    if (await layoutPage.viewerLink.isVisible()) {
      ...
      // Viewer is now at root path (accept both localhost and 127.0.0.1)
      expect(page.url()).toMatch(/^http:\/\/(localhost|127\.0\.0\.1):3456\/$/);
    }
  });
  ...
  test('Authentication flow works @smoke', async ({ page, authHelper }, testInfo) => {
    // Allow extra time for auth flows (modal + session sync)
    ...
    // Allow router.refresh/session revalidation to fully settle
  ...
  test('Library functionality works for authenticated users @smoke', async ({
    ...
    // Save the JSON via Share modal flow (explicitly click Save; no auto-open)
    const saveButton = page.locator('[data-testid="save-button"]');
    const dialog = page.getByRole('dialog');
    // Ensure authenticated UI is present
    await expect(saveButton).toBeVisible({ timeout: 30000 });
    // Wait until JSON is recognized (button enabled)
    await expect(saveButton).toBeEnabled({ timeout: 10000 });
    await saveButton.click();
    // Wait for dialog to appear (title varies across Save/Share modals)
    await expect(dialog).toBeVisible({ timeout: 10000 });
    // Enter a title and generate link (private by default)
    await page.fill('#title', documentTitle);

    // Prefer Share modal primary action if present, otherwise fall back to Save modal primary action
    const shareSaveButton = dialog.locator('[data-testid="share-save-button"]');
    const modalSaveButton = dialog.locator('[data-testid="save-button"]');

    if ((await shareSaveButton.count()) > 0) {
      await shareSaveButton.click();
      ...
      await dialog.locator('[data-testid="share-cancel-button"]').click().catch(() => {});
    } else {
    ...
    // Verify the saved document appears in library by reading item titles directly
    const titles = await page
      .locator('[data-testid="json-title"], [data-testid="card-title"]')
      .allTextContents();
    const hasDocumentTitle = titles.some((t) => (t || '').includes(documentTitle));
    expect(hasDocumentTitle).toBe(true);
  });
  ...
  test('Error handling works correctly @smoke', async ({ page }) => {
    ...
    // Error message should be informative (wait a bit for it to appear)
    await page.waitForTimeout(500);
