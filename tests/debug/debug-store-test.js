...
async function debugStoreState() {
  ...
  try {
    ...
    const postSwitchState = await page.evaluate(() => {
      ...
      const anyTreeContent = document.querySelector('[data-tree]') || document.querySelector('[role="tree"]');
      ...
      const isTreeTabActive = treeTabButton?.getAttribute('aria-selected') === 'true' || 
