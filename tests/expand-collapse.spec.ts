...
test.describe('JSON Viewer Expand/Collapse Functionality', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.locator('[data-testid="json-editor"]').fill(JSON.stringify(complexTestJson, null, 2));
  ...
  test('should show expand buttons only for complex fields (objects and arrays)', async ({ page }) => {
  ...
  test('should expand and collapse individual complex fields correctly', async ({ page }) => {
    ...
    const userNode = page.locator('[data-testid="json-node"]').filter({
      hasText: 'user:'
    }).first();
    ...
    await expect(userExpandButton.locator('[data-testid="chevron-right"], .lucide-chevron-right')).toBeVisible();
    
    // Click to expand user object
    await userExpandButton.click();
    
    // Should now show chevron down (expanded)
    await expect(userExpandButton.locator('[data-testid="chevron-down"], .lucide-chevron-down')).toBeVisible();
    
    // Should now see user properties
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'id:' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'name:' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'profile:' })).toBeVisible();
    
    // Find and expand the profile object
    const profileNode = page.locator('[data-testid="json-node"]').filter({
      hasText: 'profile:'
    }).first();
    ...
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'age:' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'location:' })).toBeVisible();
    
    // Test array expansion - find hobbies array
    const hobbiesNode = page.locator('[data-testid="json-node"]').filter({
      hasText: 'hobbies:'
    }).first();
    ...
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: '[0]:' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'reading' })).toBeVisible();
  });
  ...
  test('should handle deeply nested structures correctly', async ({ page }) => {
    ...
    // Expand user
    await page.locator('[data-testid="json-node"]').filter({ hasText: 'user:' }).first().locator('button').click();
    
    // Expand profile
    await page.locator('[data-testid="json-node"]').filter({ hasText: 'profile:' }).first().locator('button').click();
    
    // Expand location
    await page.locator('[data-testid="json-node"]').filter({ hasText: 'location:' }).first().locator('button').click();
    
    // Expand coordinates
    const coordinatesNode = page.locator('[data-testid="json-node"]').filter({ hasText: 'coordinates:' }).first();
    await coordinatesNode.locator('button').click();
    
    // Should now see the lat and lng properties
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'lat:' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'lng:' })).toBeVisible();
    
    // Verify the values are visible
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: '40.7128' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: '-74.006' })).toBeVisible();
  });
  ...
  test('should handle arrays with complex objects correctly', async ({ page }) => {
    ...
    const postsNode = page.locator('[data-testid="json-node"]').filter({ hasText: 'posts:' }).first();
    await postsNode.locator('button').click();
    
    // Should see array indices
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: '[0]:' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: '[1]:' })).toBeVisible();
    
    // Expand the first post object
    const firstPostNode = page.locator('[data-testid="json-node"]').filter({ hasText: '[0]:' }).first();
    await firstPostNode.locator('button').click();
    
    // Should see post properties
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'title:' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'tags:' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'metadata:' })).toBeVisible();
    ...
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'tech' })).toBeVisible();
    await expect(page.locator('[data-testid="json-node"]').filter({ hasText: 'programming' })).toBeVisible();
  });
});