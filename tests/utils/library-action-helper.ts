...
export class LibraryActionHelper {
  ...
  async clickSaveToLibraryButton(): Promise<boolean> {
    const saveButton = this.viewerPage.page.locator(
      '[data-testid="save-to-library"], button:has-text("Save"), button:has-text("Add to Library")'
    );
  ...
  async fillSaveForm(metadata: SaveMetadata): Promise<void> {
    // Fill title
    const titleInput = this.viewerPage.page.locator(
      '[data-testid="document-title"], input[placeholder*="title"], input[name="title"]'
    );
    ...
    if (metadata.description) {
      const descriptionInput = this.viewerPage.page.locator(
        '[data-testid="document-description"], textarea[placeholder*="description"], textarea[name="description"]'
      );
    ...
    if (metadata.tags) {
      const tagsInput = this.viewerPage.page.locator(
        '[data-testid="document-tags"], input[placeholder*="tags"], input[name="tags"]'
      );
    ...
    if (metadata.category) {
      const categorySelect = this.viewerPage.page.locator(
  ...
  async confirmSave(): Promise<boolean> {
    const confirmButton = this.viewerPage.page.locator(
      '[data-testid="save-confirm"], button:has-text("Save"), button:has-text("Confirm")'
    );
    ...
    if (await confirmButton.isVisible({ timeout: 2000 })) {
      ...
      try {
        await expect(
          this.viewerPage.page.locator('.success-message, [data-testid="success-message"], .toast')
  ...
  async cancelSave(): Promise<boolean> {
    const cancelButton = this.viewerPage.page.locator(
      '[data-testid="save-cancel"], button:has-text("Cancel")'
    );
    ...
    if (await cancelButton.isVisible()) {
      ...
      try {
        await expect(
          this.viewerPage.page.locator('[data-testid="save-modal"], .modal')
  ...
  async isSaveDisabledForInvalidJSON(invalidJson: string): Promise<boolean> {
    ...
    const saveButton = this.viewerPage.page.locator(
      '[data-testid="save-to-library"], button:has-text("Save")'
    );
  ...
  async saveLargeDocument(jsonContent: string, metadata: SaveMetadata): Promise<SaveResult> {
    ...
    const confirmButton = this.viewerPage.page.locator(
      '[data-testid="save-confirm"], button:has-text("Save")'
    );
    if (await confirmButton.isVisible()) {
      await confirmButton.click();

      // Look for progress indicators
      const progressIndicator = this.viewerPage.page.locator(
        '[data-testid="save-progress"], .progress-bar, .loading-spinner'
      );
      ...
      try {
        await expect(
          this.viewerPage.page.locator('.success-message, [data-testid="success-message"]')
        ).toBeVisible({ timeout: 5000 });
        return { success: true };
      } catch {
  ...
  async verifyDocumentInLibrary(documentTitle: string): Promise<boolean> {
    ...
    const searchInput = this.libraryPage.page.locator(
    ...
    if (await searchInput.isVisible()) {
      ...
      const documentCard = this.libraryPage.page.locator(
  ...
  async attemptDuplicateSave(
    ...
    if (await this.clickSaveToLibraryButton()) {
      ...
      if (await confirmButton.isVisible()) {
        ...
        const duplicateWarning = this.viewerPage.page.locator(
          '[data-testid="duplicate-warning"], .duplicate-error'
        );
        if (await duplicateWarning.isVisible()) {
          ...
          const successMsg = this.viewerPage.page.locator(
