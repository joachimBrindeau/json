...
async function testExpandFunctionality() {
  ...
  try {
    ...
    const initialExpandButtons = await page.$$eval(
    ...
    try {
      ...
      const afterUserExpand = await page.$$eval(
        'svg.lucide-chevron-right, svg.lucide-chevron-down',
        (elements) => elements.length
      );
      ...
      await page.screenshot({
        path: 'test-results/expand-test-02-user-expanded.png',
        fullPage: true,
      });
    } catch (e) {
    ...
    try {
      ...
      if (profileButton) {
        ...
        const afterProfileExpand = await page.$$eval(
          'svg.lucide-chevron-right, svg.lucide-chevron-down',
          (elements) => elements.length
        );
        ...
        await page.screenshot({
          path: 'test-results/expand-test-03-profile-expanded.png',
          fullPage: true,
        });
    ...
    try {
      ...
      await page.screenshot({
        path: 'test-results/expand-test-04-arrays-expanded.png',
        fullPage: true,
      });
    } catch (e) {
    ...
    // Final count
    const finalExpandButtons = await page.$$eval(
      'svg.lucide-chevron-right, svg.lucide-chevron-down',
      (elements) => elements.length
    );
    ...
    console.log('\n🎯 TEST RESULTS:');
    console.log(
      `✅ Multiple expand buttons (${finalExpandButtons} >= 5): ${success.hasMultipleExpandButtons}`
    );
